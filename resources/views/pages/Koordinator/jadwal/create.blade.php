                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('jadwal.store')}}" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="datetime-local" name="waktu" id="waktu" class="form-control @error('Waktu') is-invalid @enderror" placeholder="Masukkan Tanggal" value="{{ old('waktu') ? \Carbon\Carbon::parse(old('waktu'))->format('Y-m-d\TH:i') : '' }}">
                                @error('waktu')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="penguji1" name="penguji1" class="select2 form-control" required>
                                        ...
                                        @foreach ($dosen as $item)
                                            <option 
                                                value="{{ $item['user_id'] }}" 
                                                {{ old('penguji1') == $item['user_id'] ? 'selected' : '' }}
                                            >
                                                {{ $item['nama'] }}
                                            </option>
                                        @endforeach
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="penguji2" name="penguji2" class="select2 form-control" required>
                                        ...
                                        @foreach ($dosen as $item)
                                            <option 
                                                value="{{ $item['user_id'] }}" 
                                                {{ old('penguji2') == $item['user_id'] ? 'selected' : '' }}
                                            >
                                                {{ $item['nama'] }}
                                            </option>
                                        @endforeach
